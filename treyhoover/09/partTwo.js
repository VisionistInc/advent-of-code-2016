...
const unzip = (str, multiplier = 1) => {
  ...
  return prev + compounded + carriedForward;
};

lines.forEach(line => {
  const unzipped = unzip(line);
  console.log('unzipped', unzipped.length);
});
